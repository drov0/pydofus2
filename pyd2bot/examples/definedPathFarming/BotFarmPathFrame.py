...
class BotFarmPathFrame(Frame):
    ...
    def pushed(self) -> bool:
        ...
        return True
    def getNextCoords(self,a):
        for l in self.path:
            if a == l:
                index= self.path.index(l)
        ...
        if isinstance(msg, InteractiveUseErrorMessage):
            ...
            if msg.elemId == self._currentRequestedElementId:
                ...
                if self._currentRequestedElementId == -1:
                    ...
                    MoveAPI.MapChange(x,y)
                    self.pathIndex=(self.pathIndex + 1)%len(self.path)
            return True
