...
def main(pydofus2_dir):
    with open(ROOTDIR / "template.j2", "r") as f:
        ...
        for name, msg in tqdm(json_spec["type"].items()):
            ...
            path_to = os.path.join(*([pydofus2_dir] + msg["package"].split(".")))
...
if __name__ == "__main__":
    pydofus2_dir = sys.argv[1]
    if not os.path.exists(pydofus2_dir):
        raise FileNotFoundError(f"pydofus2 directory {pydofus2_dir} not found!")
    main(pydofus2_dir)
