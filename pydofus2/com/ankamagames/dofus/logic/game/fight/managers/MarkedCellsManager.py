...
from com.ankamagames.atouin.managers.EntitiesDisplayManager import EntitiesDisplayManager
from com.ankamagames.atouin.managers.SelectionManager import SelectionManager
from com.ankamagames.atouin.renderers.CellLinkRenderer import CellLinkRenderer
from com.ankamagames.atouin.renderers.TrapZoneRenderer import TrapZoneRenderer
from com.ankamagames.atouin.types.CellLink import CellLink
...
from com.ankamagames.dofus.network.enums.GameActionMarkCellsTypeEnum import GameActionMarkCellsTypeEnum
...
class MarkedCellsManager(IDestroyable, metaclass=Singleton):
    ...
    def addMark(self, markCasterId:float, markId:int, markType:int, associatedSpell:Spell, associatedSpellLevel:SpellLevel, cells:list[GameActionMarkedCell], teamId:int = 2, markActive:bool = True, markImpactCellId:int = -1) -> None:
        if not self._marks[markId] or self._marks[markId].len(cells) == 0:
            mi = MarkInstance()
            ...
            mi.selections = list[Selection](0, False)
            mi.cells = list[int](0, False)
            mi.teamId = teamId
            ...
            if len(cells) > 0:
                ...
                SelectionManager().addSelection(s, self.getSelectionUid(), markedCell.cellId)
                for cell in s.cells:
    ...
    def getMarks(self, pMarkType:int, pTeamId:int, pActiveOnly:bool = True) -> list[MarkInstance]:
        ...
        for mi in self._marks:
            if (pMarkType == 0 or mi.markType == pMarkType) and (pTeamId == TeamEnum.TEAM_SPECTATOR or mi.teamId == pTeamId) and (not pActiveOnly or mi.active):
                marks.append(mi)
        return marks
        ...
        for s in selections:
            ...
            if self._marks[markId].markType == GameActionMarkTypeEnum.TRAP:
                for cell in s.cells:
                    ...
                    if cellIndex != -1:
                        DataMapProvider().obstaclesCells.splice(cellIndex, 1)
    ...
    def removeGlyph(self, markId:int) -> None:
        if self._glyphs[markId]:
            if isinstance(self._glyphs[markId], Glyph):
                del self._glyphs[markId]
            else:
                del self._glyphs[markId].clear()
            del self._glyphs[markId]
    ...
    def getMarksMapPoint(self, markType:int, teamId:int = 2, activeOnly:bool = True) -> list[MapPoint]:
        ...
        for mi in self._marks.values():
            if mi.markType == markType and (teamId == TeamEnum.TEAM_SPECTATOR or mi.teamId == teamId) and (not activeOnly or mi.active):
    ...
    def getActivePortalsCount(self, teamId:int = 2) -> int:
        count:int = 0
        for mi in self._marks:
            if mi.markType == GameActionMarkTypeEnum.PORTAL and (teamId == TeamEnum.TEAM_SPECTATOR or mi.teamId == teamId) and mi.active:
                count += 1
        return count
        ...
        if pEvent.propertyName == "transparentOverlayMode":
            for markId in self._marks:
                ...
                if pEvent.propertyValue == True:
                    ...
                    strata = mi.markType == int(PlacementStrataEnums.STRATA_PORTAL) if GameActionMarkTypeEnum.PORTAL else int(PlacementStrataEnums.STRATA_GLYPH)
                for selection in mi.selections:
                    selection.renderer.strata = strata
                    selection.update(True)
                glyphs = self._glyphs[markId]
                if isinstance(glyphs, Glyph):
                    EntitiesDisplayManager().displayEntity(glyphs, glyphs.position, strata, False)
                else:
                    for glyph in glyphs:
                        EntitiesDisplayManager().displayEntity(glyph, glyph.position, strata, False)
            portalsLinks = SelectionManager().getSelection("eliaPortals")
            if portalsLinks:
                for cellLink in portalsLinks.renderer:
                    cellLink.display(int(strata) if pEvent.propertyValue == True else int(PlacementStrataEnums.STRATA_LINK))
    ...
    def updateMarksNumber(self, marktype:int) -> None:
        markInstanceToNumber:list = list()
        teamIds:list = list()
        for mi in self._marks:
        ...
        for teamId in teamIds:
            markInstanceToNumber[teamId].sortOn("markId", list.NUMERIC)
            num = 1
            for mitn in markInstanceToNumber[teamId]:
                if self._glyphs[mitn.markId]:
                    ...
                    if isinstance(self._glyphs[mitn.markId], Glyph):
                        Glyph(self._glyphs[mitn.markId]).addNumber(num, color)
                num += 1
