...
class FightEntitySlideStep(AbstractSequencable, IFightStep):
    ...
    def start(self) -> None:
        if self._entity:
            ...
            if not self._entity.position == self._startCell:
                logger.warn(f"We were ordered to slide {self._fighterId} from {self._startCell.cellId}, but self fighter is on {self._entity.position.cellId}.")
    ...
    def slideFinished(self) -> None:
        ...
        FightEventsHelper.sendFightEvent(FightEventEnum.FIGHTER_SLIDE, [self._fighterId], self._fighterId, self.castingSpellId)
