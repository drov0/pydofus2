...
class SellerStateEnum:
    ...
    EXCHANGE_OPEN_REQUEST_SENT = 7
    EXCHANGE_OPEN = 9
    EXCHANGE_ACCEPT_SENT = 10
    

class BotExchangeGiveAllFrame(Frame):
    PHENIX_MAPID = None
    
    def __init__(self):
        super().__init__()

    def pushed(self, target: dict) -> bool:
    ...
    def process(self, msg: Message) -> bool:
        ...
        if isinstance(msg, AutoTripEndedMessage):
            ...
            if msg.source == self.target["id"]:
                self.state = SellerStateEnum.EXCHANGE_OPEN_REQUEST_SENT
