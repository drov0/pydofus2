...
class FightEntitiesFrame(AbstractEntitiesFrame, Frame):
    ...
    def getEntityInfos(self, entityId: float) -> GameFightFighterInformations:
        return super().getEntityInfos(entityId)
    ...
    def isEntityOnCell(self, cellId: int) -> bool:
        return any(infos.disposition.cellId == cellId for infos in self._entities.values())
    ...
    def process(self, msg: Message) -> bool:
        ...
        if isinstance(msg, GameFightRefreshFighterMessage):
            ...
            if fullInfos:
                ...
                if (
                    krnl.Kernel().worker.contains("FightPreparationFrame")
        ...
        for ent in self._entities:
            if ent.contextualId == targetId:
                ...
                if self._tempFighterList != None and len(self._tempFighterList) != 0 and index != -1:
                    del self._tempFighterList[index]
                return
    ...
    def updateStatedElement(self, se: StatedElement) -> None:
        if not MapDisplayManager().isIdentifiedElement(se.elementId):
            Logger().error(
                f"Unknown identified element ({se.elementId}) unable to change its state to ({se.elementState}) !"
            )
            return
    ...
    def getOrdonnedPreFighters(self) -> list[float]:
        entitiesIds = self.getEntitiesIdsList()
        fighters = list[float]()
    ...
    def updateFighter(self, fighterInfos: GameFightFighterInformations) -> None:
        lastInvisibilityStat = 0
        fighterId = fighterInfos.contextualId
        if fighterInfos.spawnInfo.alive:
            lastInvisibilityStat = -1
            lastFighterInfo = self.entities.get(fighterId)
            if lastFighterInfo:
